...
var (
	defaultNetworkPath = "/var/run/akt-docker/network/network/"
...
// Connect 连接容器到之前创建的网络 akt-docker run -net testnet -p 8080:80 xxxx
func Connect(networkName string, info *container.Info) error {
	...
	// 配置端口映射信息，例如 akt-docker run -p 8080:80
